...
# --- NEW: Background task for sending heartbeats ---
async def send_heartbeat(websocket):
    try:
        while True:
            await asyncio.sleep(5)
            # Toggle heartbeat between 0 and 1
            robot_state["heartbeat"] = 1 if robot_state["heartbeat"] == 0 else 0
            
            payload = {
                "type": "STATS",
                "heartbeat": robot_state["heartbeat"],
                "message": "Heartbeat updated"
            }
            # print(f"[Heartbeat] {payload}") # Uncomment if you want to log every heartbeat
            await websocket.send(json.dumps(payload))
    except websockets.exceptions.ConnectionClosed:
        pass # The main handler will catch the disconnect
# --------------------------------------------------
...
async def handler(websocket):
    ...
    # --- NEW: Start the heartbeat task when a client connects ---
    ...
    try:
        ...
        # --- NEW: Stop the heartbeat task if the client leaves ---
